    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (request.getParameter("btnRegistrarProfe") != null && request.getParameter("rProfesor") != null) {
                ...
                profeDto = profeDao.validarUsuario(request.getParameter("txtUser"), request.getParameter("txtPass"));
                if (profeDto.getClaveProfesor() != 0) {
                    HttpSession miSesion = request.getSession(true);
                    miSesion.setAttribute("usuarioLogueado", profeDto);
                    response.sendRedirect("pages/listado.jsp");
                } else {
                    response.sendRedirect("pages/login.jsp?msg=Usuario No existe!!");
                }
